...
impl<'a> ProtoRead<'a> for Cursor<&'a [u8]> {
    fn read_wire(&mut self) -> (usize, Wire<'a>) {
        ...
        (id, match ty {
            WireType::VarInt => Wire::VarInt(self.read_var_i32()),
            WireType::Fixed64 => Wire::Fixed64(self.read_u64::<LittleEndian>()),
            WireType::Len => {
                let len = self.read_var_i32() as usize;
                let pos = self.position();
                info!("len = {}, pos = {}", len, pos);
                self.check_remaining(len).expect("buffer underflow");
                self.skip(len);
                struct Cur<'a> {
                    buf: &'a [u8],
                    pos: usize,
                ...
                let this: &'a Cur = unsafe { core::mem::transmute(self) };
                Wire::Len(&this.buf[pos..(pos + len)])
            }
            WireType::Fixed32 => Wire::Fixed32(self.read_u32::<LittleEndian>()),
        })
    }
}